...
## career_fights_updated.csv: fake placeholder rows + fragmented fighter identities

**Discovered while:** cross-referencing `data/round_stats.parquet` (8SI v2 Stage 1.1) coverage against `data/ufc-master.csv`'s 2015+ fights — coverage came in well under the round-data ingestion's own 98% target, which led to auditing why.

**Bug 1 — 69 fighter names, ~1,104 rows, 100% fabricated.** A single canned 16-fight history (opponents James Saunders, Michael Imperato, Jon Williams, Allan Wilson, Hakeem Dawodu, Ousmane Thomas Diagne, ..., dated 2011-04-16 through 2025-10-18) was stamped verbatim onto 69 different "fighter" identities in `career_fights_updated.csv`, including several of the sport's most prominent names — Cris Cyborg, Rampage Jackson, Mirko Cro Cop, Minotauro Nogueira, Katlyn Chookagian, Tecia Torres. Proof it's fabricated, not a real shared regional record: the 69 names span completely incompatible eras, weight classes, and genders (a 2000s heavyweight legend and a modern female featherweight cannot share an identical 16-opponent record). `ufc-master.csv` has real records for these fighters (e.g. Cris Cyborg's actual fights vs. Amanda Nunes, Felicia Spencer) — only `career_fights_updated.csv`, which feeds `compute_career_stats()`/`compute_qa_stats()`/`compute_got_finished_rate()`, was affected. Likely residue from an incomplete prior patch pass (`experiments/archive/patch_retired_unfixed.py`, `sherdog_fix.py` — the file's own `_updated` suffix suggests a fix that didn't fully complete).

**Bug 2 — 24 pairs, real fighters split across two name spellings** (nickname, typo, capitalization/hyphenation, name-order convention, or a formal name change), unrelated to Bug 1. `compute_career_stats()` groups by the literal fighter string, so each half of a split identity gets its own truncated, wrong career history. 10 pairs had byte-identical fight-history signatures under both spellings (unambiguous literal duplication); 14 had genuinely different rows under each spelling (a real split career).

**Detection, and why it's trustworthy:** exact-signature matching (bulletproof for the 10 duplicate pairs) plus normalized fuzzy name matching corroborated by shared-opponent overlap for the rest — with the shared-opponent signal computed only after excluding both the 69 fake names and a cluster of ~16 "opponent" values that turned out to themselves be placeholder junk (each referenced by 100+ distinct "fighter" rows, a statistically impossible fight count for a real UFC opponent — the fake-history template's own 16 opponents, in fact). Several plausible-looking candidates were individually verified against their actual opponent lists and **rejected** as dangerous false positives this same process would otherwise have merged: Anderson Silva/Wanderlei Silva and Nate Diaz/Nick Diaz (distinct famous fighters coincidentally sharing many era-mate opponents — Anderson/Wanderlei share 7 opponents, just below the accepted bar of 8), Randy Couture/Ryan Couture (real father/son), Jake/Joe Ellenberger (likely brothers), Erick Silva/Erik Silva (two provably non-overlapping careers despite a 95.2 fuzzy-name score — the single case here that would have been accepted on name similarity alone, which is exactly why fuzzy score alone was never treated as sufficient), plus four more common-surname coincidences.

**Fix (`training/fix_fighter_identities.py`):** delete the 69 fake names' rows outright (affected fighters fall back to the trainer's existing missing-data defaults until real history is reconstructed — a separate future task; `round_stats.parquet` has real per-round data for their 2015+ fights a future pass could use). Rename the 24 pairs' non-canonical spelling to whichever spelling `round_stats.parquet`'s ufcstats.com source uses, in both `career_fights_updated.csv` (fighter + opponent columns) and `ufc-master.csv` (R_fighter/B_fighter), dropping rows that become exact duplicates as a result.

**Result:** 1,104 fake rows removed, 693 cells renamed + 400 duplicate rows collapsed in `career_fights_updated.csv`; 80 cells renamed in `ufc-master.csv`. Pooled walk-forward log loss **improved** (not just got cleaner): 0.6152062 → 0.6134525 (−0.0018) — confirming this restored real signal rather than just removing noise. Single-holdout accuracy 69.17% → 70.00%. `round_stats.parquet` coverage of 2015+ `ufc-master.csv` fights rose from 95.96% to 97.24% (still short of the 98% Stage 1.1 target — the residual gap is fighters genuinely absent from ufcstats.com's own scrape, e.g. Katlyn Chookagian has zero rows under any spelling in `round_stats.parquet`, not a reconciliation failure). All 104 tests pass.
