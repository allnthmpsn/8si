#!/usr/bin/env python3
"""
8SI UFC Predictor — Women's Model 2A Trainer
  Blend:    50% LogisticRegression + 50% XGBoost
  Features: M1 base (106) + odds features
  Scope:    Women's fights with complete moneyline odds only
  ...
  Fallback: If <300 odds-complete fights, writes metadata stub and exits
"""
import sys
import os
import gc
...
# 109-feature base (same as M1 before zero-rate drop)
FEAT_BASE = [
...
# Odds features added by M2A
FEAT_ODDS = [
    "R_impl_prob", "B_impl_prob", "impl_prob_dif",
    "market_fav_R",
    "R_dec_impl", "B_dec_impl", "R_ko_impl", "B_ko_impl",
    "R_sub_impl", "B_sub_impl",
]
...
def ml_to_impl_prob(series):
    ...
    return pd.Series(prob, index=series.index)
...
def corner_flip(X, y, w=None):
    ...
    for col in Xf.columns:
        if col.endswith('_dif'):
            Xf[col] = -Xf[col]
    # flip odds features
    for r_col, b_col in [('R_impl_prob','B_impl_prob'),('R_dec_impl','B_dec_impl'),
                          ('R_ko_impl','B_ko_impl'),('R_sub_impl','B_sub_impl')]:
        if r_col in Xf.columns and b_col in Xf.columns:
            tmp = Xf[r_col].values.copy()
            Xf[r_col] = Xf[b_col].values
            Xf[b_col] = tmp
    if 'market_fav_R' in Xf.columns:
        Xf['market_fav_R'] = 1 - Xf['market_fav_R']
    if 'impl_prob_dif' in Xf.columns:
        Xf['impl_prob_dif'] = -Xf['impl_prob_dif']
...
def train_and_eval(X_train_raw, y_train_raw, d_train_raw, X_test, y_test, feat_cols, hl):
    ...
    return lr, xgb, y_pred, acc


def main():
    print('=' * 62)
    print('  8SI UFC Predictor — Women\'s Model 2A Trainer')
    print(f'  Blend: {int(LR_WEIGHT*100)}% LR + {int(XGB_WEIGHT*100)}% XGB')
    print(f'  Scope: women\'s fights with complete odds | Cutoff: {TRAIN_CUTOFF}')
    print('=' * 62)
    ...
    # ── 2b. Odds filter — check threshold ────────────────────────────────────
    odds_mask = (
        master_womens['R_odds'].notna() & master_womens['B_odds'].notna() &
        (master_womens['R_odds'] != 0) & (master_womens['B_odds'] != 0)
    )
    n_odds_complete = odds_mask.sum()
    print(f'   Fights with complete moneyline odds: {n_odds_complete:,}')

    if n_odds_complete < MIN_ODDS_FIGHTS:
        print(f'\n   ⚠  Only {n_odds_complete} fights have complete odds < threshold ({MIN_ODDS_FIGHTS}).')
        print(f'   Skipping M2A training — falling back to M1 only.')
        metadata_stub = {
            'model2a': 'insufficient data — using M1 only',
            'n_odds_complete': int(n_odds_complete),
            'threshold': MIN_ODDS_FIGHTS,
            'date_checked': datetime.now().strftime('%Y-%m-%d'),
        }
        out_path = os.path.join(MODEL, 'ufc_model2a_womens_metadata.json')
        with open(out_path, 'w') as fh:
            json.dump(metadata_stub, fh, indent=2)
        print(f'   Wrote stub metadata → {os.path.relpath(out_path, ROOT)}')
        return

    master_womens = master_womens[odds_mask].copy().reset_index(drop=True)
    print(f'   Proceeding with {len(master_womens):,} odds-complete fights')
    print(f'   Year range: {master_womens["date"].dt.year.min()} – {master_womens["date"].dt.year.max()}')

    # ── 3. Elo (women's fights only — from full women's set, not odds-filtered) ─
    print('\n[3/9] Computing Elo from all women\'s fights (K=48, base=1500)...')
    all_womens = df_all[df_all['weight_class'].isin(WOMENS_CLASSES) &
                        df_all['Winner'].isin(['Red','Blue'])].copy()
    all_womens = all_womens.sort_values('date').reset_index(drop=True)
    elo_hist_df, elo_curr_df = compute_elo_womens(all_womens, K=48, base=1500.0)
    ...
    print('\n[6/9] Engineering features (base + odds)...')
    ...
    # ── Odds features ──────────────────────────────────────────────────────────
    df['R_impl_prob']   = ml_to_impl_prob(df['R_odds'])
    df['B_impl_prob']   = ml_to_impl_prob(df['B_odds'])
    df['impl_prob_dif'] = df['R_impl_prob'] - df['B_impl_prob']
    df['market_fav_R']  = (df['R_impl_prob'] > 0.5).astype(int)
    df['R_dec_impl']    = ml_to_impl_prob(df.get('r_dec_odds', pd.Series(0, index=df.index)))
    df['B_dec_impl']    = ml_to_impl_prob(df.get('b_dec_odds', pd.Series(0, index=df.index)))
    df['R_ko_impl']     = ml_to_impl_prob(df.get('r_ko_odds',  pd.Series(0, index=df.index)))
    df['B_ko_impl']     = ml_to_impl_prob(df.get('b_ko_odds',  pd.Series(0, index=df.index)))
    df['R_sub_impl']    = ml_to_impl_prob(df.get('r_sub_odds', pd.Series(0, index=df.index)))
    df['B_sub_impl']    = ml_to_impl_prob(df.get('b_sub_odds', pd.Series(0, index=df.index)))
    ...
    # ── 8. Zero-rate check on base features ──────────────────────────────────
    print(f'\n[8/9] Zero-rate check on base features (threshold: {ZERO_RATE_THRESHOLD*100:.0f}%)...')
    train_mask = df['date'] < TRAIN_CUTOFF
    feat_all = FEAT_BASE + FEAT_ODDS
    X_full_train = df.loc[train_mask, FEAT_BASE]
    zero_rates = (X_full_train == 0).mean()
    dropped_base = zero_rates[zero_rates > ZERO_RATE_THRESHOLD].index.tolist()
    kept_base = [f for f in FEAT_BASE if f not in dropped_base]
    kept_feats = kept_base + FEAT_ODDS
    print(f'   Base features dropped ({len(dropped_base)}):')
    for feat in dropped_base:
        print(f'     {feat}: {zero_rates[feat]*100:.1f}% zero')
    print(f'   Base kept: {len(kept_base)} | Odds features added: {len(FEAT_ODDS)}')
    print(f'   Total features: {len(kept_feats)}')
    ...
    for hl in [730, 1095]:
        print(f'\n   ─── Training HL={hl}d ───')
        lr_m, xgb_m, y_pred, acc = train_and_eval(
            X_train_raw, y_train_raw, d_train_raw, X_test, y_test, kept_feats, hl
        )
        results[hl] = (lr_m, xgb_m, y_pred, acc)
    ...
    lr_best, xgb_best, y_pred_best, acc_best = results[best_hl]
    ...
    # ── Save outputs ───────────────────────────────────────────────────────────
    print('\n   Saving outputs (women\'s M2A only — other model files untouched)...')

    joblib.dump(lr_best,    os.path.join(MODEL, 'ufc_model2a_womens_lr.pkl'))
    joblib.dump(xgb_best,   os.path.join(MODEL, 'ufc_model2a_womens_xgb.pkl'))
    joblib.dump(kept_feats, os.path.join(MODEL, 'ufc_model2a_womens_features.pkl'))

    acc_730  = results[730][3]
    acc_1095 = results[1095][3]
    metadata = {
        'model_type': f'M2A_womens_blend_LR50_XGB50_HL{best_hl}d',
        'temporal_accuracy': round(float(acc_best), 6),
        'n_features': len(kept_feats),
        'feature_list': kept_feats,
        'dropped_base_features': dropped_base,
        'odds_features': FEAT_ODDS,
        ...
        'training_window': f'All available (odds-complete) → <{TRAIN_CUTOFF}',
        'test_window': f'>= {TRAIN_CUTOFF} (women\'s odds-complete fights only)',
        'fight_filter': 'R_cum_fights >= 1 AND B_cum_fights >= 1 AND complete odds',
    ...
    saved = [
        os.path.join(MODEL, 'ufc_model2a_womens_lr.pkl'),
        os.path.join(MODEL, 'ufc_model2a_womens_xgb.pkl'),
        os.path.join(MODEL, 'ufc_model2a_womens_features.pkl'),
        out_path,
    ]
    for path in saved:
    ...
    print(f'\n{"=" * 62}')
    print(f'  Training complete (Women\'s M2A).')
    ...
    print(f'  Features: {len(kept_base)} base + {len(FEAT_ODDS)} odds = {len(kept_feats)} total')
    print(f'{"=" * 62}\n')
